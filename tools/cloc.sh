...
# This scripts counts the lines of code and comments in all source files. The
# copyright-headers are substracted. It uses the command line tool "cloc". You can
# either pass --loc, --comments or --percentage to show the respective values only.
...
SUMMARY="$(cloc --vcs=git --include-lang="Rust" --md | tail -1)"
...
# lines. As cloc does not count inline comments, the overall estimate should be
# rather conservative.
